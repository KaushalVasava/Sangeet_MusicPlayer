...
//Add to playlist dailog
fun addToPlaylistDialog(context: Context, container: ViewGroup?, lists:ArrayList<Songs>, position: Int)
{
    val playlistDialog = LayoutInflater.from(context)
        .inflate(R.layout.playlist_dialog, container, false)
    ...
    listView.onItemClickListener = AdapterView.OnItemClickListener { parent, view, pos, id ->
        if (pos == getList.size - 1) {
            ...
            val introText = "New Playlist ${musicPlaylist.ref!!.size + 1}"
                ...
                .setPositiveButton("Create") { dialog, _ ->
                    ...
                    if (playlistName != null) {
                        if (playlistName.isNotEmpty()) {
                            ...
                            if (available) {
                                musicPlaylist.ref!![pos].playlist.add(lists[position])
                                Toast.makeText(
                                    context,
                                    "Song added to playlist ${musicPlaylist.ref!![pos].name}",
                                    Toast.LENGTH_SHORT
                                ).show()
                            }
                        }
            ...
            var isExist = false
            for (i in musicPlaylist.ref!![pos].playlist) {
                //item already exist
                if (i.id == lists[position].id) {
                    Toast.makeText(
                        context,
                        "Song already in the playlist ${musicPlaylist.ref!![pos].name}",
                        Toast.LENGTH_SHORT
                    ).show()
                    isExist = true
                    break
...
fun addToPlaylist(context: Context,playlistName: String): Boolean {
...
fun checkHistorySong(id: Int, list: ArrayList<Songs>, position: Int) {
    val hsize = historySongList.size
    var isFound = false
    for (i in 0 until hsize) {
        if (id == historySongList[i].id) {
            historySongList.removeAt(i)
            historySongList.add(0, list[position])
            isFound = true
            break
        }
    }
...
fun getFavoriteIndex(id: Int): Int {
    isFavorite = false
    favoriteList!!.forEachIndexed { index, song ->
        if (id == song.id) {
            isFavorite = true
            return index
        }
