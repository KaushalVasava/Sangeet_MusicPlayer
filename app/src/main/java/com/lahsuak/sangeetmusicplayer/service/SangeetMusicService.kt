package com.lahsuak.sangeetmusicplayer.service
...
import com.lahsuak.sangeetmusicplayer.receiver.MusicNotificationReceiver
...
class SangeetMusicService : Service(), OnCompletionListener {
...
//old one
    ...
    inner class MyBinder : Binder() {
        val service: SangeetMusicService
            get() = this@SangeetMusicService
    }
    ...
    fun createMediaPlayer(innerPosition: Int, uri: String?) {
        ...
        editor.putInt(LAST_SONG_ID, songFiles!![position].id) //added to get current song in recent player
        editor.apply()
    ...
    fun showNotification(isPlay: Boolean, isFavorite: Boolean, playbackOption: Int, playbackSpeed: Int) {
        ...
        val shuffleOrRepeatPendingIntent =
            getBroadcast(this, 0, shuffleOrRepeatIntent, FLAG_UPDATE_CURRENT)

        val art = Uri.parse(songFiles!![position].artUri!!)
        var artwork: Bitmap?
        try {
            val stream = contentResolver.openInputStream(art)
            artwork = BitmapFactory.decodeStream(stream)
        } catch (e: Exception) {
            artwork = BitmapFactory.decodeResource(resources, R.drawable.ic_music)
        }

        var option = 0
        if (playbackOption == 0) {
            option = R.drawable.ic_repeat
        } else if (playbackOption == 1) {
            option = R.drawable.ic_repeat_one
        } else if (playbackOption == 2) {
            option = R.drawable.ic_shuffle
        }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            ...
            mediaSession!!.setPlaybackState(
                PlaybackStateCompat.Builder()
                    .setState(
                        PlaybackStateCompat.STATE_PLAYING, mediaPlayer!!.currentPosition.toLong(),
    ...
    fun isPlaying(): Boolean {
        ...
        editor.putBoolean(PLAY_STATUS, mediaPlayer!!.isPlaying)
        editor.apply()
